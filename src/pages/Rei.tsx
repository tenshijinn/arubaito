...
export default function Rei() {
    ...
    const restoreTwitterState = async () => {
      try {
        ...
        if (storedTwitterUser) {
          ...
          if (storedVerification) setVerificationStatus(JSON.parse(storedVerification));
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (twitterUser && step === 1) setStep(2);
  }, [twitterUser]);

  useEffect(() => {
    if (connected && twitterUser && step === 2) setStep(3);
  ...
  const handleTwitterLogin = async (mode: 'signin' | 'signup') => {
    try {
      ...
      toast({ title: 'Error', description: 'Failed to initiate Twitter login', variant: 'destructive' });
    }
  };
  ...
  const handleTwitterCallback = async (code: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('twitter-oauth', {
        body: { action: 'exchangeToken', code, codeVerifier: storedVerifier, redirectUri },
      });
  ...
  const handleAudioReady = (blob: Blob) => {
    setAudioBlob(blob);
    toast({ title: 'Audio Ready', description: 'Your audio introduction is ready to submit' });
  };
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('submit-rei-registration', {
        body: {
          ...
          consent: true,
          reanalyze: useExistingTranscript,
        },
      });
      ...
      if (data && data.registration) {
        ...
        toast({
          title: 'Success!',
          description: useExistingTranscript ? 'Profile re-analyzed!' : (isEditMode ? 'Profile updated!' : (data.message || 'Registration successful!')),
        });
      } else {
  ...
  const handleWhitelistRequest = async () => {
    ...
    try {
      ...
      toast({ title: data.success ? 'Request Submitted!' : 'Already Submitted', description: data.message });
    } catch (error) {
      toast({ title: 'Error', description: 'Failed to submit whitelist request', variant: 'destructive' });
    } finally {
      ...
      <div className="w-full md:w-1/2 min-h-screen flex flex-col">
        <div className="flex-1 flex flex-col items-center justify-center overflow-y-auto px-6 py-10 md:p-[100px]">
          ...
          <div className="w-full max-w-2xl mb-8">
            <h1 style={{ fontSize: '30px', fontWeight: 300, color: '#f0ede8', letterSpacing: '-0.025em', marginBottom: '8px' }}>
              Rei Proof-Of-Talent Portal
            </h1>
            <div className="flex items-center gap-2">
              {[1, 2, 3].map((s) => (
                <div key={s} className="flex-1">
                  <div style={{
                    height: '3px',
                    borderRadius: '2px',
                    background: s <= step ? '#e8c4b8' : '#1e1e1e',
                ...
                {!twitterUser ? (
                  showSignUp ? (
                    <div className="space-y-6">
                      ...
                      <p className="text-center" style={{ fontSize: '13px', color: '#5c5a57' }}>
                        Already have an account?{' '}
                        <button onClick={() => setShowSignUp(false)} style={{ fontWeight: 500, color: '#f0ede8', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: '3px' }}>
                          Sign in
                        </button>
                      </p>

                      {noAccountFound && (
                        <div className="rei-surface-2 flex items-center gap-3" style={{ padding: '14px', borderColor: 'hsla(0,63%,55%,0.3)' }}>
                          <AlertCircle className="h-4 w-4" style={{ color: '#ef4444' }} />
                          <span style={{ fontSize: '13px', color: '#ef4444' }}>No existing account found. Please sign up.</span>
                        </div>
                      )}
                    </div>
                  ) : (
                    <div className="space-y-6">
                      <div className="space-y-3">
                        ...
                        <button
                          ...
                          className="btn-manga btn-manga-outline w-full flex items-center justify-center gap-2"
                          style={{ borderRadius: '28px', padding: '11px 22px', cursor: 'pointer' }}
                        >
                          <Twitter style={{ width: '16px', height: '16px' }} />
                      ...
                      <p className="text-center" style={{ fontSize: '13px', color: '#5c5a57' }}>
                        ...
                        <button onClick={() => setShowSignUp(true)} style={{ fontWeight: 500, color: '#f0ede8', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: '3px' }}>
                          Sign up
                        </button>
                      </p>
                  ...
                  <div className="space-y-3">
                    <div className="rei-surface-2 flex items-center gap-3" style={{ padding: '14px' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <p style={{ fontWeight: 500, color: '#f0ede8', fontSize: '14px' }}>{twitterUser.display_name}</p>
                        <p style={{ fontSize: '12px', color: '#5c5a57' }}>@{twitterUser.handle}</p>
                      </div>
                      {twitterUser.verified && (
                        <span className="rei-chip" style={{ padding: '3px 10px', fontSize: '10px' }}>Verified</span>
                      )}
                    </div>
                    <div className="rei-surface-2 flex items-center gap-2" style={{ padding: '10px 14px', borderColor: 'hsla(18,52%,82%,0.22)' }}>
                      <Check className="h-4 w-4" style={{ color: '#e8c4b8' }} />
                      <span style={{ fontSize: '13px', fontWeight: 500, color: '#e8c4b8' }}>Identity Verified</span>
                    </div>
                  </div>
                )}
              </div>
                ...
                <div className={step !== 2 && connected ? 'opacity-40' : ''}>
                  <div className="flex items-center gap-3 mb-4">
                    <div className="h-7 w-7 rounded-full flex items-center justify-center" style={{
                      background: connected ? 'hsla(18,52%,82%,0.12)' : '#1e1e1e',
                      fontSize: '12px',
                      color: connected ? '#e8c4b8' : '#5c5a57',
                      border: '0.5px solid hsla(0,0%,100%,0.08)',
                    }}>
                      {connected ? <Check className="h-3.5 w-3.5" /> : '2'}
                    </div>
                  ...
                  {connected && publicKey && (
                    <div className="rei-surface-2 mt-3" style={{ padding: '14px' }}>
                      <p style={{ fontSize: '11px', color: '#5c5a57', marginBottom: '4px' }}>Connected Wallet</p>
                      <p style={{ fontSize: '11px', fontFamily: "'SF Mono', 'Consolas', monospace", color: '#a09e9a', wordBreak: 'break-all' }}>
                        {publicKey.toString()}
                      </p>
                    </div>
              ...
              {twitterUser && connected && (!isSuccess || isEditMode) && (
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span style={{ fontSize: '14px', fontWeight: 500, color: '#f0ede8' }}>
                      {isEditMode ? 'Edit My Details' : 'Submit My Details'}
                    </span>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {isEditMode && registrationData?.file_path && (
                      <div className="rei-surface-2 space-y-3" style={{ padding: '14px', borderColor: 'hsla(18,52%,82%,0.22)' }}>
                        <div className="flex items-center gap-2">
                          ...
                          <span style={{ fontSize: '13px', fontWeight: 500, color: '#f0ede8' }}>Re-analyze Profile</span>
                        </div>
                        <p style={{ fontSize: '12px', color: '#5c5a57', lineHeight: '1.65' }}>
                          Re-run AI analysis with updated wallet data. No need to record again!
                        </p>
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            ...
                            onChange={(e) => {
                              setUseExistingTranscript(e.target.checked);
                              if (e.target.checked) setAudioBlob(null);
                            }}
                            style={{ accentColor: '#e8c4b8' }}
                          />
                          <span style={{ fontSize: '13px', color: '#a09e9a' }}>Use existing introduction</span>
                        </label>
                      </div>
                    )}
                    ...
                    {!useExistingTranscript && (
                      <div>
                        <div className="rei-section-label">
                          {isEditMode ? 'Record New Introduction' : 'Record Your Introduction'}
                        </div>
                    ...
                    {/* Portfolio URL */}
                    <div>
                      <div className="rei-section-label">Portfolio URL (Optional)</div>
                      <input
                        type="url"
                        placeholder="https://..."
                        value={portfolioUrl}
                    ...
                    <div className="rei-surface-2" style={{ padding: '14px' }}>
                      <label className="flex items-start gap-2 cursor-pointer">
                        <input
                          ...
                          style={{ accentColor: '#e8c4b8', marginTop: '2px' }}
                        />
                        <span style={{ fontSize: '13px', color: '#a09e9a' }}>I consent to data storage *</span>
                      </label>
                    </div>
                    ...
                    <div className="flex gap-2">
                      {isEditMode && (
                        <button
                          onClick={() => { setIsEditMode(false); setAudioBlob(null); setUseExistingTranscript(false); }}
                          className="btn-manga btn-manga-outline flex-1"
                          style={{ borderRadius: '28px', padding: '11px 22px', cursor: 'pointer' }}
                        >
                          Cancel
                        </button>
                      ...
                      <button
                        ...
                        className="btn-manga btn-manga-primary flex-1"
                        style={{
                          borderRadius: '28px',
                          padding: '11px 22px',
                      ...
                      >
                        {isSubmitting
                          ...
                          : useExistingTranscript
                            ? 'Re-analyze Profile'
                            : (isEditMode ? 'Update Profile' : 'Register & Claim NFT')}
                      </button>
                    </div>
                  </div>
                </div>
