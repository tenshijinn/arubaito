...
export const LatestUpdatesCarousel = () => {
  ...
  return <div className="w-full flex flex-col items-center">
      ...
      <div ref={emblaRef} className="overflow-hidden w-full max-w-md">
        <div className="flex">
          {updates.map(update => <div key={update.id} className="flex-[0_0_100%] min-w-0 flex flex-col items-center">
              {/* ASCII Art */}
              <iframe src={update.asciiSrc} className="w-full aspect-square mb-8 border-0" style={{
            backgroundColor: "transparent"
              ...
              color: "#d0d0d0"
                ...
                {update.link && <a href={update.link} target="_blank" rel="noopener noreferrer" className="inline-block mt-2 text-xs font-mono hover:opacity-80 transition-opacity" style={{
              color: "#ed565a"
            }}>
                    Learn more →
                  </a>}
      ...
      <div className="flex gap-2 mt-6">
        {updates.map((_, index) => <button key={index} onClick={() => emblaApi?.scrollTo(index)} className={cn("w-2 h-2 rounded-full transition-all duration-300", selectedIndex === index ? "w-6" : "opacity-40")} style={{
