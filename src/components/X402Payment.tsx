...
export const X402Payment = ({ amount, memo, onSuccess, onCancel }: X402PaymentProps) => {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const { data: paymentData, error: createError } = await supabase.functions.invoke('x402-create-payment', {
        body: {
          amount,
          memo,
      ...
      // Step 3: Verify and submit transaction
      const { data: verifyData, error: verifyError } = await supabase.functions.invoke('x402-verify-payment', {
        body: {
          ...
          <div className="bg-muted/50 rounded-lg p-4 space-y-2">
            ...
            {memo && (
              <div className="flex justify-between text-sm">
                ...
                <span className="font-mono text-xs">{memo.slice(0, 20)}...</span>
              </div>
            )}
          </div>
        </div>
